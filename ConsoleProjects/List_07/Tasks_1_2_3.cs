...
namespace List_07
{
    internal class Tasks_1_2_3
    {
        ...
        public static void ConvertStudentsWithTopicsToStudentsAB(List<StudentWithTopics> studentsWithTopics)
        {
            /* Solution (a) - minimalistic, topics list hardcoded */
            ...
            /* Solution (b) - topics list generated by the query */
            int idCounter = 1;
        ...
        public static void ConvertStudentsWithTopicsToStudentsC(List<StudentWithTopics> studentsWithTopics)
        {
            int idCounter = 1;
            var topicsList = (from distinctTopic in (
                                (from stud in studentsWithTopics
            ...
            var students = from studWithTopics in studentsWithTopics
                           select new StudentWithoutTopics(
                               studWithTopics.Id,
                               studWithTopics.Index,
                               studWithTopics.Name,
                               studWithTopics.Gender,
            ...
            var studentsToTopics = from studWithTopics in studentsWithTopics
                                   from topic in studWithTopics.Topics
                                   select new StudentToTopic(studWithTopics.Id, getTopicsIdBasedOnName(topic, topicsList));
            ...
            Console.WriteLine("\nStudents mapped to topics in a sepparate list [StudentsToTopics]");
            foreach (var studTopic in studentsToTopics)
            {
                Console.WriteLine(studTopic);
            }

            Console.WriteLine("\nStudents after conversion[StudentsWithoutTopics]");
            foreach (var stud in students)
            {
                Console.WriteLine(stud);
            }
